...
public class PrefixStrings {
    public static Boolean isPrefixStrings(String[] a, String[] b) {
        ...
        for (var w : b) {
            if (!isPrefixStrings(w, a, data))
                return false;
        }
        return true;
    }

    public static boolean isPrefixStrings(String w, String[] a, Map<String, Boolean> data) {
        if (data.containsKey(w))
            return data.get(w);

        for (int i = 1; i <= w.length(); i++) {
            for (String word : a) {
                if (w.startsWith(word) && isPrefixStrings(w.substring(i), a, data)) {
                    data.put(w, true);
                    return true;
                }
            }
